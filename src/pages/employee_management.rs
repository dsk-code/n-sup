...
pub fn EmployeeManagement() -> impl IntoView {
    let (employees, set_employees) = signal(vec![
        ...
        Employee {
            ...
            position: "営業".to_string(),
            email: "suzuki@nsup.com".to_string(),
            phone: "090-3333-4444".to_string(),
            ...
            <div class="container mx-auto px-4 py-8">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                    ...
                    >
                        ...
                        <span class="hidden sm:inline">"新規従業員追加"</span>
                    </button>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-slate-700/50">
                                <tr>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-300">"氏名"</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-300">"従業員ID"</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-300">"部署"</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-300">"役職"</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-300">"メール"</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-300">"電話"</th>
                                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-300">"入社日"</th>
                                    ...
                                    children=move |employee| {
                                        ...
                                        view! {
                                            <tr class="hover:bg-slate-700/30 transition-colors duration-200">
                                                <td class="px-4 py-3 font-medium">{employee.name.clone()}</td>
                                                <td class="px-4 py-3 text-slate-300 text-sm">{employee.employee_id.clone()}</td>
                                                <td class="px-4 py-3 text-slate-300 text-sm">{employee.department.clone()}</td>
                                                <td class="px-4 py-3 text-slate-300 text-sm">{employee.position.clone()}</td>
                                                <td class="px-4 py-3 text-slate-300 text-xs">{employee.email.clone()}</td>
                                                <td class="px-4 py-3 text-slate-300 text-xs">{employee.phone.clone()}</td>
                                                <td class="px-4 py-3 text-slate-300 text-sm">{employee.hire_date.clone()}</td>
                                                <td class="px-4 py-3">
                                                    <span class={format!("px-2 py-1 rounded-full text-xs font-medium {}", status_color(&employee.status))}>
                            ...
                            view! {
                                <div class="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 p-4">
                                    <div class="flex justify-between items-start mb-3">
                                        <div>
                                            <h3 class="font-semibold text-white mb-1">{employee.name.clone()}</h3>
                                            <p class="text-slate-300 text-sm">{employee.employee_id.clone()}</p>
                                        </div>
                                        <span class={format!("px-2 py-1 rounded-full text-xs font-medium {}", status_color(&employee.status))}>
                                            {status_text(&employee.status)}
                                        </span>
                                    ...
                                    <div class="grid grid-cols-2 gap-2 mb-4 text-sm">
                                        <div>
                                            <span class="text-slate-400">"部署: "</span>
                                            <span class="text-slate-300">{employee.department.clone()}</span>
                                        </div>
                                        <div>
                                            <span class="text-slate-400">"役職: "</span>
                                            <span class="text-slate-300">{employee.position.clone()}</span>
                                        </div>
                                        <div>
                                            <span class="text-slate-400">"入社: "</span>
                                            <span class="text-slate-300">{employee.hire_date.clone()}</span>
                                        </div>
                                        <div>
                                            <span class="text-slate-400">"電話: "</span>
                                            <span class="text-slate-300">{employee.phone.clone()}</span>
                                        </div>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <span class="text-slate-400 text-sm">"メール: "</span>
                                        <span class="text-slate-300 text-sm">{employee.email.clone()}</span>
                                    </div>
